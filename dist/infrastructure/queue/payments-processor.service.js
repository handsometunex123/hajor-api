...
var PaymentsProcessorService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.PaymentsProcessorService = void 0;
...
let PaymentsProcessorService = PaymentsProcessorService_1 = class PaymentsProcessorService {
    constructor(prisma, transactions, fraud) {
        this.prisma = prisma;
        this.transactions = transactions;
        this.fraud = fraud;
        this.logger = new common_1.Logger(PaymentsProcessorService_1.name);
    }
    async process(job, queues) {
        this.logger.log(`Processing payments job ${job.id} name=${job.name}`);
        if (job.name === 'auto-debit-cycle')
            return this.processAutoDebit(job, queues);
        if (job.name === 'retry-failed-payments')
            return this.processRetryFailed(job, queues);
        if (job.name === 'grace-period-reminder')
            return this.processGracePeriodReminder(job, queues);
        this.logger.warn(`Unknown payments job: ${job.name}`);
        return { ok: false };
    }
    async processAutoDebit(job, queues) {
        ...
        for (const p of unpaidPayments) {
            ...
            try {
                ...
                await this.prisma.$transaction(async (tx) => {
                    ...
                    if (!groupId)
                        throw new Error('Group not found for cycle');
                    ...
                    const de = await this.transactions.createDoubleEntry({ fromWalletId: wallet.id, toWalletId: groupWallet.id, amount, reference: txRef, status: 'SUCCESS', metadata: { cycleId, paymentId } }, tx);
                ...
                this.logger.error(`Auto-debit error for payment ${paymentId}: ${err === null || err === void 0 ? void 0 : err.message}`);
                try {
                    await this.prisma.contributionPayment.update({ where: { id: paymentId }, data: { status: 'FAILED' } });
                }
                catch (_) { }
                results.push({ paymentId, status: 'failed_error', error: err === null || err === void 0 ? void 0 : err.message });
            }
        }
        const failedUserIds = new Set(results
            .filter((r) => { var _a; return (_a = r.status) === null || _a === void 0 ? void 0 : _a.startsWith('failed'); })
            .map((r) => { var _a, _b; return (_b = (_a = unpaidPayments.find((p) => p.id === r.paymentId)) === null || _a === void 0 ? void 0 : _a.groupContributor) === null || _b === void 0 ? void 0 : _b.userId; })
            .filter(Boolean));
        for (const uid of failedUserIds) {
            try {
                await this.fraud.checkDefaultRate(uid);
            }
            catch (_) { }
        }
    ...
    async processRetryFailed(job, queues) {
        const { cycleId } = job.data;
        if (!cycleId)
            throw new Error('Missing cycleId');
        ...
        for (const p of failedPayments) {
            try {
                ...
                const userId = p.groupContributor.userId;
                const wallet = await this.prisma.wallet.findUnique({ where: { userId } });
                if (!wallet || wallet.paystackProvisionStatus !== 'PROVISIONED')
                    continue;
                const credit = await this.prisma.transaction.aggregate({ _sum: { amount: true }, where: { walletId: wallet.id, type: 'CREDIT', status: 'SUCCESS' } });
                const debit = await this.prisma.transaction.aggregate({ _sum: { amount: true }, where: { walletId: wallet.id, type: 'DEBIT', status: 'SUCCESS' } });
                const balance = (credit._sum.amount ? Number(credit._sum.amount.toString()) : 0)
                ...
                this.logger.error(`Retry debit failed for ${p.id}: ${err === null || err === void 0 ? void 0 : err.message}`);
            }
        }
    ...
    async processGracePeriodReminder(job, queues) {
        var _a;
    ...
    async checkCycleCompletion(cycleId, contributionDate, queues) {
        try {
            const paymentsAll = await this.prisma.contributionPayment.findMany({ where: { cycleId } });
            const allPaid = paymentsAll.length > 0 && paymentsAll.every((p) => p.status === 'PAID');
            if (allPaid) {
            ...
            this.logger.error(`Error checking cycle completion: ${err === null || err === void 0 ? void 0 : err.message}`);
        }
    }
};
exports.PaymentsProcessorService = PaymentsProcessorService;
exports.PaymentsProcessorService = PaymentsProcessorService = PaymentsProcessorService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        transactions_service_1.TransactionsService,
        fraud_service_1.FraudService])
], PaymentsProcessorService);
//# sourceMappingURL=payments-processor.service.js.map